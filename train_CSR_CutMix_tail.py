    ...
    for images, targets in loader:
        ...
        if use_cutmix:
            images_cm, y_mix, y_a, y_b, lam = cutmix_aug(
                ...
                beta=cutmix_beta, apply_prob=cutmix_prob
            )
            ...
            with torch.no_grad():
                ...
                w_fair = lam * gm[preds, y_a] + (1.0 - lam) * gm[preds, y_b]
        else:
            logits = model(images)
            loss_ce = F.cross_entropy(logits, targets, label_smoothing=label_smoothing, reduction="mean")
            with torch.no_grad():
                preds = logits.argmax(dim=1)
                w_fair = gm[preds, targets]

        if fair_mode == "wce":
            if use_cutmix:
                # 用两端硬标签的 per-sample CE 按 λ 混合，再乘以 gm 权重
                per_a = F.cross_entropy(logits, y_a, reduction="none", label_smoothing=0.0)
                per_b = F.cross_entropy(logits, y_b, reduction="none", label_smoothing=0.0)
                per_mix = lam * per_a + (1.0 - lam) * per_b
                loss_fair = (w_fair * per_mix).mean()
            else:
                per_sample = F.cross_entropy(logits, targets, reduction="none", label_smoothing=label_smoothing)
                loss_fair = (w_fair * per_sample).mean()
        else:
            # KL：对目标分布整体加权（CutMix 时用 y_mix，非 CutMix 用 onehot）
            if use_cutmix:
                yy = y_mix * w_fair.unsqueeze(1)
            else:
                yy = F.one_hot(targets, num_classes=num_classes).float() * w_fair.unsqueeze(1)
