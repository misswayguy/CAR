...
def build_dataloaders(cfg: TrainConfig):
    ...
    if cfg.use_balanced_sampler:
        ...
        # # 可选：做一次均值归一，避免整体尺度影响训练；并加下界避免极端值
        # w = (w / w.mean()).clamp(min=1e-3)

        # # 放到设备上备用（如果你在多卡DDP里用的是 local_rank 对应的 device）
        # w = w.to(device)
...
#     # 构造非 j 的掩码（True 表示非 j）
#     mask_nonj = ~F.one_hot(targets, C).to(device).bool()      # [B,C]
#     S_nonj = S_nonj * mask_nonj.float()                       # 不用 scatter_，避免 in-place
...
#     gate = gate * mask_nonj.float()                           # 同理，避免 gate.scatter_
...
def spectral_loss_from_C(C: torch.Tensor, w: torch.Tensor) -> torch.Tensor:
    """
    L_spec = || C @ diag(w) ||_2 = 最大奇异值
    """
    Cw = C @ torch.diag(w.to(C.device))
    # svd 可反传（奇异值不相等处几乎处处可导）
...
def train_one_epoch(
    ...
    temp: float = 1.0,
    label_smoothing: float = 0.0,
    detach_gate_delta: bool = True,           # ← 新增
    detach_softmax_baseline: bool = True,     # ← 新增
    ablate_no_sigmoid: bool = False,
    ablate_no_softmax: bool = False,
    ablate_no_both: bool = False,
    gamma: float = 0.0,
    ...
    spec_time = 0.0
    """
    可导谱正则 + EMA 的训练一个 epoch。
    - cbar: 持续的 C_bar（会被更新并返回）；None 时将用首个 batch 初始化。
    """
    model.train()
...
def main():
    ...
    N = float(counts_t.sum().item()) + 1e-12                  # 总样本数
    m = counts_t / N                                          # 频率 m_j ∈ [0,1]

    # 频率尺度的 r0（主路径）
    if args.r0 is not None:
        w = 1.0 / torch.sqrt(m + float(args.r0) + 1e-12)

    # 兼容：如果只给了旧的 --lambda0，则退回到“样本数+大常数”的老公式
    ...
    for epoch in range(1, cfg.epochs + 1):
        # 只在训练后 20% 的 epoch 记录梯度（符合 reviewer 的关切）
        ...
        #     # >>> 关键改动：右乘 diag(w) 再做 SVD
        #     if args.use_wj:
        #         # S_eff = S_ema @ diag(w)    （放大列=真类维度；与你论文里的 || C^{col-norm} W ||_2 一致）
        ...
        train_loss, C_bar, train_time, spec_time = train_one_epoch(
            ...
            detach_gate_delta=args.detach_gate_delta,                  # ← 从 args 传入
            detach_softmax_baseline=args.detach_softmax_baseline,       # ← 从 args 传入
        ...
        # ---------------- Evaluation ----------------
        # 1) 训练集评估（使用 train_loader_eval：与验证同分布的中心裁剪，避免数据增强干扰指标）
        ...
        # 日志：同时报出 Train 与 Val 的 WCA（以及可选的总体 Acc / Macro 指标）
        log = (f"Epoch {epoch:03d} | TrainLoss {train_loss:.4f} | "
